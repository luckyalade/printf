	...
	int length, char padd, char extra_c)
{
	...
	if (prec == 0 && ind == BUFF_SIZE - 2 && buffer[ind] == '0')
		buffer[ind] = padd = ' '; /* width is displayed with added padding ' ' */
 ...
 * @width: Width specifier function
 * @flags: Flags specifier function
	...
	int width, int flags, char padd, char extra_c, int padd_start)
{
	...
	if (width > length)
	{
		...
		else if (!(flags & F_MINUS) && padd == ' ')/* additional char to left of buffer */
		{
			buffer[--ind] = 'x';
			buffer[--ind] = '0';
			if (extra_c)
		...
		else if (!(flags & F_MINUS) && padd == '0')/* additional char to left of padd */
		{
			if (extra_c)
				buffer[--padd_start] = extra_c;
			buffer[1] = '0';
